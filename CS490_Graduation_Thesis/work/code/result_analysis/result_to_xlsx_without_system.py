...
def pick_out_score(results):
    ...
    for result in results:
        ...
        # 使用正则表达式匹配评分和总分
        score_patterns = [
            # 匹配 "得分：8", "得分为：8.5", "得分：**8", "最终打分：<8>" 等
            r"(?:得分|总分|评分|总评|打分) {0,1}\*{0,2} {0,1}为{0,1}(?:：|: |:|:) {0,1}\*{0,2} {0,1}<{0,1} {0,1}(\d+(?:\.\d+)?)",
        ]
        scores = []
...
if __name__ == '__main__':
    ...
    folder_list = [
        'all_system-w-o-alls_few-shot-0',
        'all_system-holistic_few-shot-0',
        'all_system-holistic_few-shot-1',
        'all_system-holistic_few-shot-2',
        'all_system-holistic_few-shot-3',
    ]
