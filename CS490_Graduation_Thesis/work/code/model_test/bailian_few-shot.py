...
def score_report(path, model_detail, max_count, max_example):
    """
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith(".txt"):
                ...
                scores = get_score(model_detail, text, max_example)
...
def txt(model_info, path, limitation, prompt_limit, saving):
    """
    将模型的评分结果进行格式重构，输出成txt文件
    """
    print(f"start to get score of model {model_info[0]}")
    results = score_report(path, model_info[1], limitation, prompt_limit)
...
if __name__ == '__main__':
    ...
    for folder in folder_list:
        ...
        for i in range(max_few_shot_count):
            ...
            for model in models:
                txt(model, input_path, fileCount, few_shot_count,
                    rf"{output_path}/{model[0]}/{folder}_few-shot-{few_shot_count}")
        print(f"处理完成：{folder}")
