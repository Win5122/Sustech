...
public class MinioUtils {
     ...
     */
    public List<String> upload_multi(MultipartFile[] multipartFile) {
    ...
    public List<String> upload_multi(MultipartFile[] multipartFile, String path) {
        List<String> names = new ArrayList<>(multipartFile.length);
        for (MultipartFile file : multipartFile) {
            String fileName = file.getOriginalFilename();
            String[] split = fileName.split("\\.");
    ...
    public byte[] downloadBytes(String fileName, String path) {
        ...
        try {
            ...
            if (fileName.endsWith(".jpeg")) {
                bytes = downloadBytes("0.jpeg", path);
//                responseEntity = download("0.jpeg", path);
            }
        } finally {
            try {
                if (in != null) {
...
//        return responseEntity;
    ...
    public ResponseEntity<byte[]> downloadResponseEntity(String fileName, String path) {
        fileName = path + "/" + fileName;
        ResponseEntity<byte[]> responseEntity = null;
        InputStream in = null;
        ByteArrayOutputStream out = null;
        try {
            ...
            responseEntity = downloadResponseEntity("0", path);
        } finally {
            try {
                if (in != null) {
                    try {
...
//            throw new RuntimeException("upload failed");
     ...
     * @return java.lang.String
     */
    public String upload_reduce(MultipartFile multipartFile, String path, String name) {
        name = path + "/" + name;
        InputStream in = null;
        try {
